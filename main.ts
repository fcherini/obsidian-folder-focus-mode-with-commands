...
import { Plugin, WorkspaceLeaf, setIcon, TFile, TFolder } from "obsidian";
...
export default class FolderFocusModePlugin extends Plugin {
	...
	shouldBeVisible(
		...
		const upElement =
			parentsArray.length >= 2
				? parentsArray[parentsArray.length - 2]
				: "/";
		...
		fileExplorers.forEach((fileExplorer: ExplorerLeaf) => {
			...
			for (const key in fileExplorer.view.fileItems) {
				if (fileExplorer.view.fileItems.hasOwnProperty(key)) {
					...
					this.substituteSetterForElement(
						fileExplorer.view.fileItems[key]
					);

					if (shouldBeVisible) {
						fileExplorer.view.fileItems[key].el.classList.remove(
							"hidden-tree-element"
						);
					} else {
						fileExplorer.view.fileItems[key].el.classList.add(
							"hidden-tree-element"
						);
					}
			...
			const existingButton =
				FolderFocusModePlugin.getFocusButton(fileExplorer);
			if (existingButton) {
		...
		fileExplorers.forEach((fileExplorer: ExplorerLeaf) => {
			for (const key in fileExplorer.view.fileItems) {
				if (fileExplorer.view.fileItems.hasOwnProperty(key)) {
					...
					fileExplorer.view.fileItems[key].el.classList.remove(
						"folderfocus-up-parent"
					);
				}
			}

			fileExplorer.view.containerEl
				.querySelector(".folderfocus-up-element-link")
				?.classList?.remove("folderfocus-up-element-link");
	...
	getDirRoot(file: TFile) {
		if (this.settings.focusNote) {
			const linkedFolder = this.app.vault.getAbstractFileByPath(
				file.path.replace(".md", "")
			);
			if (linkedFolder && linkedFolder instanceof TFolder) {
				return linkedFolder.path;
			} else {
				return this.settings.autofocusRoot
					? getRootDirname(file.path)
	...
	registerFolderCommands() {
		const commandFolders = this.settings.folderCommands;
		commandFolders.forEach((folder, i) => {
			const commandId = `folder-focus-${i}`;
			this.addCommand({
				id: commandId,
				name: `Focus on: ${folder}`,
				callback: () => {
					this.hideTreeElements(folder);
				},
			});
		});
	}
	...
	initialiseFocusButton(forcedValue?: boolean) {
		const settingCheck = forcedValue
			? forcedValue
			: this?.settings?.focusButton;
	...
	initialiseUpButton() {
		...
		explorers.forEach((explorer) => {
			this.registerDomEvent(
				explorer.view.containerEl,
				"click",
				(evt: MouseEvent) => {
					// get the folder path
					const elemTarget = evt.target as HTMLElement;
					const realTarget = elemTarget.closest(
						".folderfocus-up-element-link"
					) as HTMLElement;
	...
	initialiseFileContextMenu(menu: any, file: any) {
		if (!file?.extension) {
			...
			menu.addItem((item: any) => {
				item.setTitle(
					isCurrentlyFocused ? "Unfocus" : "Focus on this folder"
				)
					.setIcon("eye")
					.onClick(async () =>
						this.toggleFocusOption(isCurrentlyFocused, file.path)
					);
			});
		} else {
			// create context menu for files - only if allowed
			const isCurrentlyFocused =
				this.focusModePath === file?.parent?.path;
			if (this.settings.fileContextMenu) {
				menu.addItem((item: any) => {
					item.setTitle(
						isCurrentlyFocused ? "Unfocus" : "Focus on this file"
					)
						.setIcon("eye")
						.onClick(async () =>
							this.toggleFocusOption(
								isCurrentlyFocused,
								file?.parent?.path
							)
						);
	...
	*/
	...
	async onload() {
		...
		this.registerEvent(
			this.app.workspace.on("layout-change", this.initialiseFocusButton)
		);

		// initialise context menu event
		this.registerEvent(
			this.app.workspace.on("file-menu", (menu, file) =>
				this.initialiseFileContextMenu(menu, file)
			)
		);
		...
		this.registerEvent(
			this.app.workspace.on("layout-change", this.initialiseUpButton)
		);
		...
		this.registerEvent(
			this.app.workspace.on("file-open", (file) => {
				...
				const shouldHandleForRegularMode =
					!this.settings.autofocusForced &&
					!this.shouldBeVisible(this.focusModePath, file.path);
				const shouldHandleForForcedMode = this.settings.autofocusForced;
				const shouldHandleAutofocus =
					this.settings.autofocusMode &&
					(shouldHandleForRegularMode || shouldHandleForForcedMode);
				...
				if (shouldHandleAutofocus) {
					...
					explorers.forEach((exp) => {
						...
						const existingButton =
							FolderFocusModePlugin.getFocusButton(exp);
		...
		// initialise global command for all folders added in settings
		this.registerFolderCommands();
	...
	resetClasses() {
		...
		explorers.forEach((fileExplorer: WorkspaceLeaf) => {
			if (this.focusModeEnabled) {
				fileExplorer.view.containerEl.classList.add(
					"folder-focus-mode"
				);
			} else {
				fileExplorer.view.containerEl.classList.remove(
					"folder-focus-mode"
				);
			}
	...
	private addFocusFolderButton(explorer: WorkspaceLeaf): void {
		...
		this.registerDomEvent(newIcon, "click", () => {
			...
			if (currentFile) {
				const isCurrentlyFocused =
					this.focusModePath === currentFile.path;
				if (
					isCurrentlyFocused ||
					newIcon.classList.contains("focus-close")
				) {
					this.showAllTreeElements();
					this.setUnfocusedIconButton(newIcon);
